...
fn render_bench(c: &mut Criterion) {
    ...
    let policy = WorksheetPolicy {
        ...
        composition: vec![
            Band { source: BandSource::TargetNode, weight: 0.7, item_types: vec!["drill".into()] },
            Band { source: BandSource::Prerequisites, weight: 0.2, item_types: vec!["drill".into()] },
            Band { source: BandSource::NonRoutine, weight: 0.1, item_types: vec!["drill".into()] },
        ],
        item_count: 20,
    ...
    c.bench_function("render_20_item_math", |b| {
        b.iter(|| {
            ...
            let spec = Composer::compose(&plugin, policy.clone(), profile.clone(), &mut rng).unwrap();
